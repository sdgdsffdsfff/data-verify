...
public class HtmlUnit {
	...
	private static Logger logger = LoggerFactory.getLogger(HtmlUnit.class);
	...
	public String makeRequest(String u) throws IOException, HttpException {
		...
		request.setAdditionalHeader("Accept-Language", "en-US,en;q=0.8,zh-CN;q=0.6,zh;q=0.4,zh-TW;q=0.2");
		...
		if (code == 200) {
			...
			logger.warn("400 Bad request: " + u);
		} else if (code == 401 || code == 403) {
			logger.warn("401 Authentication Required: " + u);
		} else if (code == 404) {
			logger.warn("404 not found: " + u);
		} else if (code == 410) { // permanently GONE
			logger.warn("410 Permanently gone: " + u);
		} else if (code == 502 || code == 504) {
			logger.warn("Bad Gateway: " + u);
		} else if (code == -2) {
			logger.warn("Connection reset: " + u);
		} else {
			logger.warn(u + " return code: " + code);
		}
